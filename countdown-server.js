...
function getData() {
  ...
  try {
    ...
    // Running countdown
    if (data.status === "running" && data.targetTime > 0) {
      const now = Date.now();
      ...
      const endTime = data.targetTime + data.duration;
...
app.post("/api/admin/update", (req, res) => {
  const { newTarget, duration } = req.body;
...
app.post("/api/admin/pause", (req, res) => {
  ...
  if (now >= data.targetTime) {
    remaining = Math.max(0, data.targetTime + data.duration - now);
  }
...
app.post("/api/admin/resume", (req, res) => {
  ...
  const targetTime = Date.now() + remaining;
